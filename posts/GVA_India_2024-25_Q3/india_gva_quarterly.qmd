...
quarterly <- read_xlsx(
  ...
  mutate(Date = as.Date(paste0(as.numeric(substr(Year, start = 1, stop = 4)), "-", case_when(Quarter == 1 ~ "01",    # Q1 starts on April 1. But R need to see Q1
          Quarter == 2 ~ "04",    # as coming before Q4 which starts on Jan 1 of
          Quarter == 3 ~ "07",    # the next year.
          Quarter == 4 ~ "10"), "-01"))) |>
...
-   Summary of the latest quarter: **It was slightly better than usual. Total inflation-adjusted output (*GVA*) grew *faster* than its long-run average (that is, the average since Quarter `r min(quarterly_gva$Quarter[quarterly_gva$Date==min(quarterly_gva$Date)])` of the `r min(quarterly_gva$Year)` fiscal year). The *primary* and the *tertiary sectors* grew *faster* than their respective long-run average growth rates. The *secondary* sector grew *slower* than its long-run average growth rate.**
